...
const products = new Contenedor(path)
    ...
    global.userAuth
        ? (req.params.id
            ? products.getById(req.params.id)
                .then(result => res.send(result || { error: 'product inexistente' }))
...
api.post('/', (req, res) => {
    if (global.userAuth === 1) {
        ...
        products.addProduct(req.body)
            .then((result) => {
                if (result === 'ok') { res.redirect('../success.html') }
...
api.route('/:id')
    ...
    .put((req, res) => {
        if (global.userAuth === 1) {
            products.modifyProduct(req.body)
                .then((result) => {
                    if (result === 'ok') { res.redirect('../../success.html') }
                    else (res.redirect('../../error.html'))
                })
