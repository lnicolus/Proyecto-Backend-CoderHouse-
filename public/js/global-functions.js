...
const error = (data, $div) => {
    ...
    let error = `
            <div class="flex flex-col items-center w-full">
                ...
                <b>Descripción: ${data.description}</b>
            </div>`
    $div.innerHTML = error
}
// products.html
                ...
                <div class="lg:w-1/4 md:w-1/2 p-4 w-full hover:bg-zinc-200 content-center rounded-md transition duration-300 text-center">
                    <button onclick="getProduct('${product.id}')" >
                        ...
                        <a class="block relative rounded overflow-hidden">
                            <div class="mt-4">
                                <h3 class="text-gray-500 text-xs tracking-widest title-font mb-1">cod: ${product.code}</h3>
...
const addProductBtn = (div) => {
    ...
    addButton.innerHTML = `<button onclick="addProduct()" class="w-auto px-3 py-1 mr-4 bg-neutral-300 hover:bg-yellow-400 transition duration-300 font-bold text-black rounded">Add New Sales Item</button>`
    addButton.href = '../edit.html'
...
const showProduct = (data, a) => {
    ...
    document.getElementById('code').innerHTML = `Código: ${data.code}`
    document.getElementById('thumbnail').src = data.thumbnail
    document.getElementById('price').innerHTML = `$${data.price}`
    document.getElementById('stock').innerHTML = `${data.stock} ${data.price === 1 ? 'unidad' : "unids."}`
}
// product.html
    ...
    fetch('./api/cart', {
        ...
        body: JSON.stringify({
            ...
            product: {
                ...
                code: Number(sessionStorage.getItem('code')),
                ...
                quantity: Number(sessionStorage.getItem('quantity'))
            }
        })
    })
        .then(res => res.json())
        .then(id => { // Almaceno el id en caso de no registrar cart
