...
fetch(`./api/products/${tempId}`)
    ...
    .then(data => {
        ...
        if (data.error === -1) { error(data, $product) } //! Error
        else {
            if (editMode === '0') { $form.action = `/api/products` } //! Agregar
            else {
                document.getElementById('edit').action = `/api/products/${tempId}`
            ...
            if (editMode === '2') { //! Borrar
                $form.action = `/api/products/${tempId}`
